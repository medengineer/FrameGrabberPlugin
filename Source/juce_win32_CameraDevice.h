...
#define COMCALL virtual HRESULT STDMETHODCALLTYPE
...
{
    COMCALL SampleCB (double, IMediaSample*) = 0;
    COMCALL BufferCB (double, BYTE*, long) = 0;
};

interface ISampleGrabber  : public IUnknown
{
    COMCALL SetOneShot (BOOL) = 0;
    COMCALL SetMediaType (const AM_MEDIA_TYPE*) = 0;
    COMCALL GetConnectedMediaType (AM_MEDIA_TYPE*) = 0;
    COMCALL SetBufferSamples (BOOL) = 0;
    COMCALL GetCurrentBuffer (long*, long*) = 0;
    COMCALL GetCurrentSample (IMediaSample**) = 0;
    COMCALL SetCallback (ISampleGrabberCB*, long) = 0;
};
...
struct CameraDevice::Pimpl  : public ChangeBroadcaster
{
       ...
       : owner (ownerToUse)
    {
        ...
        LPCWSTR vc = L"Video Capture";
        hr = graphBuilder->AddFilter (filter, vc);
        if (FAILED (hr))
            return;
        ...
        LPCWSTR st = L"Smart Tee";
        hr = graphBuilder->AddFilter (smartTee, st);
        if (FAILED (hr))
            return;
        ...
        LPCWSTR sg = L"Sample Grabber";
        hr = graphBuilder->AddFilter (sampleGrabberBase, sg);
        if (FAILED (hr))
            return;
        ...
        LPCWSTR nr = L"Null Renderer";
        hr = graphBuilder->AddFilter (nullFilter, nr);
    ...
    void drawCurrentImage (Graphics& g, juce::Rectangle<int> area)
    {
        ...
        juce::Rectangle<int> centred (RectanglePlacement (RectanglePlacement::centred)
                                    .appliedTo (juce::Rectangle<int> (width, height), area));
                ...
                while (enumerator->Next (1, moniker.resetAndGetPointerAddress(), &fetched) == S_OK)
                {
                    ...
                    if (SUCCEEDED (hr))
                    {
                        ...
                        if (SUCCEEDED (hr))
                        {
                            ...
                            LPCOLESTR fn = L"FriendlyName";
                            hr = propertyBag->Read (fn, &var, 0);
                            propertyBag = nullptr;
...
private:
    ...
    bool addGraphToRot()
    {
        ...
        LPCOLESTR excl = L"!";
        HRESULT hr = CreateItemMoniker (excl, buffer, moniker.resetAndGetPointerAddress());
        if (FAILED (hr))
            return false;
